...
const toTitleCase = (arr) => {
  ...
  for(let i in arr){
    if(regex.test(arr[i]) === true){
      let toCaps = arr[i].replace(arr[i].charAt(0), arr[i].charAt(0).toUpperCase());
      capsArr.push(toCaps);
    } else {
      capsArr.push(arr[i]);
    }
...
let biggerThanLuke = (arr) => {
  let regex = /Luke Skywalker/;
  let testMass = 0;
  for(let i in arr){
    if(regex.test(arr[i].name) === true){
      testMass += parseInt(arr[i].mass);
    }
  }
  console.log(testMass);

  let people = arr.reduce( (namesList, person) => {
    if(regex.test(person.name) === false && person.mass >= testMass){
      namesList += person.name + ' - ';
    }
    return namesList;
  }, '')
...
describe('Testing challenge 4', () => {
  test.skip('It should check if url is https', () => {
...
describe('Testing challenge 5', () => {
  test.skip('It should return true if there are three in a row', () => {
    expect(detectTicTacToeWin([['X', '', 'O'], ['X', 'O', ''], ['X', 'O', 'X']])).toStrictEqual(true);
    expect(detectTicTacToeWin([['O', '', 'X'], ['X', 'O', 'X'], ['X', '', 'O']])).toStrictEqual(true);
  });

  test.skip('It should return false if there are not three in a row', () => {
    expect(detectTicTacToeWin([['X', '', 'O'], ['O', 'O', ''], ['X', 'O', 'X']])).toStrictEqual(false);
  });

  test.skip('It should not treat empty 3 in row as winner', () => {
